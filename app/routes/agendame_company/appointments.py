...
from app.models.user import User, Service, Client, Appointment
...
async def get_company_appointments(
    ...
    try:
        ...
        appointments = await Appointment.filter(query)\
...
@router.get("/agendame/appointments/today", response_model=AppointmentsListResponse)
...
@router.post("/agendame/appointments/create", response_model=AppointmentCreatedResponse)
...
async def update_appointment(
    ...
    try:
        ...
        result = await appointments_domain.update_one_appointments(
            target_appointment=appointment_id,
            schema=update_data
        )
