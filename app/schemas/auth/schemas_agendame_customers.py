...
class AgendameService(BaseModel):
    ...
    service_id: Optional[int] = Field(
        None, description='ID do serviço desejado (opcional)'
    )

    preferred_time: Optional[str] = Field(
        None, description='Horário preferido no formato HH:MM (opcional)'
    )

    total_appointments: Optional[int] = Field(
        0, description='Total de agendamentos anteriores (opcional)'
    )

    notes: Optional[str] = Field(
        None, max_length=500, description='Observações adicionais (opcional)'
    )
