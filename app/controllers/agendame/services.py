...
class Services:
    ...
    async def _get_company(self) -> MyCompany:
        ...
        try:
            if self.target_company_id:
                return await MyCompany.create(company_id=self.target_company_id)
    ...
    async def _get_company_by_name(self) -> MyCompany:
        ...
        # Primeiro tenta buscar pelo business_name
        ...
        # Se não encontrar pelo nome, tenta pelo slug (se fornecido)
    ...
    async def get_services_by_identifier(
        ...
        Este é o método principal para a rota pública que aceita diferentes
        tipos de identificação da empresa.
        """
        # Decodifica identificador (caso tenha espaços codificados na URL)
        decoded_identifier = urllib.parse.unquote(identifier)

        # Busca a empresa pelo identificador
        company = await self._get_company_by_identifier(decoded_identifier, search_type)
        ...
        if query_by and query_value is not None:
            # Valida campos permitidos para evitar injeção
            allowed_fields = ['name', 'description', 'duration_minutes', 'price', 'order']
            if query_by not in allowed_fields:
                raise ValueError(f"Campo de filtro inválido. Permitidos: {allowed_fields}")
        ...
        # Formata os resultados para a API
        services_data = []
        for service in services_models:
            services_data.append({
                'id': service.id,
                'name': service.name.strip(),
                'description': service.description.strip() if service.description else None,
    ...
    async def query(
        ...
        """
        Consulta flexível de serviços (método original mantido para compatibilidade).

        Args:
            query_by: Campo para filtrar (ex: 'name', 'duration_minutes')
            query_value: Valor do filtro
            is_active: Status dos serviços (None = todos)
            order_by: Campos para ordenação
        """
        company = await self._get_company()

        # Query base sempre filtrada pela empresa
        query = Service.filter(user_id=company.company_id())
